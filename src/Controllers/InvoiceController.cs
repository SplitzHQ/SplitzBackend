    ...
    IMapper mapper) : ControllerBase
{
    ...
    public async Task<ActionResult<InvoiceDto>> CreateInvoice(InvoiceInputDto input)
    {
        ...
        var debts = SimplifyDebts(invoice.InvoiceId, transactions);
    ...
    public async Task<IActionResult> UpdateInvoice(Guid id, InvoiceInputDto input)
    {
        ...
        var debts = SimplifyDebts(invoice.InvoiceId, newTransactions);
        context.Set<InvoiceDebt>().AddRange(debts);

        // Check if still settled
        invoice.Status = CheckIfSettled(debts, invoice.Settlements ?? new())
        ...
        InvoiceSettlementInputDto input)
    {
        ...
        invoice.Status = CheckIfSettled(invoice.Debts, allSettlements)
            ? InvoiceStatus.Settled
            : InvoiceStatus.Open;
    ...
    public async Task<IActionResult> DeleteSettlement(Guid id, Guid settlementId)
    {
        ...
        invoice.Status = CheckIfSettled(invoice.Debts, remainingSettlements)
            ? InvoiceStatus.Settled
            : InvoiceStatus.Open;
    ...
    /// </summary>
    internal static List<InvoiceDebt> SimplifyDebts(Guid invoiceId, List<Transaction> transactions)
    {
    ...
    /// </summary>
    internal static bool CheckIfSettled(IEnumerable<InvoiceDebt> debts, IEnumerable<InvoiceSettlement> settlements)
    {
    ...
    /// </summary>
    internal static async Task RecalculateInvoiceDebts(SplitzDbContext context, Guid invoiceId)
    {
        var invoice = await context.Invoices
            .Include(i => i.Transactions).ThenInclude(t => t.Balances)
            .Include(i => i.Debts)
