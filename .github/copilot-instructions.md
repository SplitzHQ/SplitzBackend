...
2. **Database Context (`SplitzDbContext`):**
    ...
    - Database managed via EF Core migrations, applied automatically on startup via `MigrateAsync()` in Program.cs
...
- **.gitignore:**
    ...
    - Excludes: bin/, obj/, _.db, _.db-shm, \*.db-wal files
...
- On app startup, `MigrateAsync()` automatically applies any pending migrations
- Test data seeding logic in `Program.cs` `SeedTestData()` method

**Migration workflow:**

```bash
# After changing models/DbContext, generate a new migration:
dotnet ef migrations add <MigrationName> --output-dir src/Migrations

# Apply migrations manually (also happens automatically on app startup):
dotnet ef database update

# Remove last unapplied migration if you made a mistake:
dotnet ef migrations remove
```

**Important notes:**

- Always specify `--output-dir src/Migrations` when adding migrations to keep them in the `src/` directory
- If you need a clean database, delete `app.db`, `app.db-shm`, and `app.db-wal`, then run the app — migrations will recreate the schema
- Never mix `EnsureCreated()` with migrations — they are incompatible
...
- Database: SQLite file-based (app.db), managed via EF Core migrations, uses Identity schema
